...
function VictoriqueTaskRow(props: Props & { task: any; active?: boolean }) {
...
export function VictoriquePanel(props: Props) {
    ...
    <CollapsiblePanel id="vic-heading" title="victorique" meta={<span className="badge badge-muted">{props.tasks.length} tasks</span>}>
      <div className="ex-grid">
        {activeTasks.length
          ...
          : <p className="state-entry">— no active victorique tasks —</p>}
        <details className="done-group ex-done-group">
          <summary>[{doneTasks.length} closed — click to expand]</summary>
          <div className="ex-grid">
            {doneTasks.map((task) => <VictoriqueTaskRow key={task.id} {...props} task={task} />)}
          </div>
        </details>
      </div>
    </CollapsiblePanel>
