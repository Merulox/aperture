...
import { getHormoziProposals, reviewHormoziProposal } from '../../lib/boreal';

export const GET: APIRoute = async () => {
  const proposals = getHormoziProposals();
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const result = reviewHormoziProposal(actionId, body.decision, (body.note ?? '').trim());
