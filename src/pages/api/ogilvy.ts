...
import { getOgilvyProposals, reviewOgilvyProposal } from '../../lib/boreal';

export const GET: APIRoute = async () => {
  const proposals = getOgilvyProposals();
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const result = reviewOgilvyProposal(actionId, body.decision, (body.note ?? '').trim());
